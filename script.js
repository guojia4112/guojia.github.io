...
// - Predicted point updates only on success; failure clears previous
// - Blue diamond with black border; black arrow
// - Async race guard via SANDBOX_RUN_ID

// ------------------ Utilities ------------------
function getById(id) { return document.getElementById(id); }

// Refresh-and-navigate helper for top nav buttons
function reloadTo(pageId) {
  try { localStorage.setItem('CURRENT_PAGE', pageId); } catch (e) {}
...
function ensureSandboxSelectedPointLayer(sandboxDiv) {
  return ensureLayer(sandboxDiv, 'SANDBOX_SELECTED_POINT', {
    ...
    hovertemplate: '%{text}<br>NOₓ: %{x}<br>O₃: %{y}', showlegend: true,
    visible: true
  });
}
      ...
      SitesAPI.load('data/sites.csv').then(function () {
        ...
        for (var s = 0; s < sites.length; s++) {
          ...
          tracesXY.push({
            ...
            hovertemplate: '%{text}<br>24NOX: %{x}<br>M1M1O3: %{y}'
          });

          var yearsNum = labels.map(function (t) { return parseInt(t, 10); });

          tracesTS.push({
            x: yearsNum, y: xs, mode: 'lines+markers', type: 'scatter', name: siteName + ' 24NOₓ',
            line: { color: color }, yaxis: 'y1', hovertemplate: '%{x}<br>24NOX: %{y}'
          });

          tracesTS.push({
            x: yearsNum, y: ys, mode: 'lines+markers', type: 'scatter', name: siteName + ' M1M1O₃',
        ...
        // 保留背景曲线：背景(VOC/NOX) + 新轨迹 一起 react
        var evoData;
        if (hasBackground(evoPlot)) {
          var bg = [];
          for (var i = 0; i < evoPlot.data.length; i++) {
            var t = evoPlot.data[i];
            if (t.legendgroup === 'VOC' || t.legendgroup === 'NOX') bg.push(t);
          }
          evoData = bg.concat(tracesXY);
        } else {
...
function initSandbox() {
  ...
  var SANDBOX_RUN_ID = (window.SANDBOX_RUN_ID || 0) + 1;
  window.SANDBOX_RUN_ID = SANDBOX_RUN_ID;

  syncFactorLabels();

  drawBackgroundIfMissing('sandboxPlot').then(function () {
    if (window.SANDBOX_RUN_ID !== SANDBOX_RUN_ID) return;
    ...
    } catch (e) { console.warn('Plotly.newPlot fallback failed:', e); }
    ...
    if (window.SitesAPI && typeof SitesAPI.load === 'function') {
      SitesAPI.load('data/sites.csv').then(function () {
        ...
        if (idxSel < 0 || idxUser < 0 || idxPred < 0) { console.error('[Sandbox] failed to ensure layers:', { idxSel: idxSel, idxUser: idxUser, idxPred: idxPred }); return; }
        ...
        if (src === 'manual') {
          ...
          if (isFinite(x) && isFinite(y)) {
            var label = 'User (' + x + ', ' + y + ')';
            Plotly.restyle(sandboxDiv, { x: [[x]], y: [[y]], text: [[label]], visible: true }, [idxUser]);
        ...
        ensureLocatorsReady().then(function (ready) {
          ...
          try {
            if (typeof window.Locator1.estimateContinuousIsoFromXY === 'function') {
              ...
              est = window.Locator1.estimateFromXY(point.x, point.y, 8);
            }
          } catch (e) { console.error('Locator1 estimate failed', e); }

          if (!est || !est.ok || !est.estimate) {
            console.warn('[Sandbox] Locator1 outside or failed:', est);
            annotations.push({
              x: point.x, y: point.y, xref: 'x', yref: 'y',
              text: 'Outside: 无法估算 (VOC, NOx)', showarrow: false,
              xshift: 10, yshift: 10, xanchor: 'left', yanchor: 'bottom',
              font: { color: '#666', size: 12 },
              bgcolor: 'rgba(255,255,255,0.9)', bordercolor: 'rgba(0,0,0,0.15)', borderwidth: 1, borderpad: 6
            });
            Plotly.restyle(sandboxDiv, { x: [[]], y: [[]], text: [[]] }, [idxPred]);
          ...
          try {
            ...
            if (xy && xy.ok && isFinite(xy.x) && isFinite(xy.y)) {
              ...
              Plotly.restyle(sandboxDiv, { x: [[px]], y: [[py]], text: [[labelPred]], visible: true }, [idxPred]);
              try { Plotly.relayout(sandboxDiv, { 'xaxis.autorange': true, 'yaxis.autorange': true }); } catch (e) {}
              annotations.push(buildArrowAnnotation(point.x, point.y, px, py));
            } else {
              console.warn('Locator2 outside/out_of_range/failed:', xy);
              Plotly.restyle(sandboxDiv, { x: [[]], y: [[]], text: [[]] }, [idxPred]);
            }
